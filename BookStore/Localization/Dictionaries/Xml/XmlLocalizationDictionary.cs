...
using BookStore.Xml.Extensions;
using System;
...
namespace BookStore.Localization.Dictionaries.Xml
{
	...
	public class XmlLocalizationDictionary : LocalizationDictionary
	{
		...
		public static XmlLocalizationDictionary BuildFomXmlString(string xmlString)
		{
			...
			{
				throw new BookStoreEVEException("A Localization Xml must include localizationDictionary as root node.");
			}

			var cultureName = localizationDictionaryNode[0].GetAttributeValueOrNull("culture");
			if (string.IsNullOrEmpty(cultureName))
			{
				throw new BookStoreEVEException("culture is not defined in language XML file!");
			}
			...
			if (textNodes != null)
			{
				foreach (XmlNode node in textNodes)
				{
					...
					{
						throw new BookStoreEVEException("name attribute of a text is empty in given xml string.");
					}
			...
			{
				throw new BookStoreEVEException("A dictionary can not contain same key twice. There are some duplicated names: " + dublicateNames.JoinAsString(", "));
			}
